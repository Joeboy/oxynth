...
pub fn audio_callback(buf: &mut [u32]) -> ControlFlow<(), ()> {
    ...
    // Pull any pending MIDI events from the consumer. Access the
    // static consumer via `unsafe` since it's a mutable static Option.
    unsafe {
        // Create a raw mutable pointer to the static and then convert it
        // to a `&mut Option<...>` to avoid the edition-2024 check that
        // forbids creating `&mut` directly from a `static mut`.
        ...
        if let Some(cons) = cons_opt.as_mut() {
            while let Some(event) = cons.dequeue() {
                // TODO: handle the MIDI event (note on/off, CC, etc.)
                // let _ = event;
